...
// ─── Browser-side Yahoo Finance fetch via CORS proxies ───────────────────────
    ...
    for (const base of [
      ...
      for (const makeProxy of corsProxies) {
        try {
          ...
          if (result) return { data: result, symbol: sym };
        } catch {
          continue;
        }
      }
...
function GreenRed({ value }: { value: string }) {
  ...
  return (
    <span
      className={`flex items-center gap-1 ${num >= 0 ? "text-green-500" : "text-red-500"}`}
    >
      {num >= 0 ? (
...
interface FundamentalsViewerProps {
  data?: any; // legacy server data (ignored now)
  stockId: number;
  symbol: string; // ADD symbol prop — pass stock.symbol from StockDetail
}

export function FundamentalsViewer({
  stockId,
  symbol,
}: FundamentalsViewerProps) {
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ["yahoo-fundamentals", symbol],
    queryFn: () => fetchFromYahoo(symbol),
      ...
      <div className="space-y-4">
        ...
        <Card>
          <CardContent className="p-6 text-center text-muted-foreground">
            Could not load fundamental data for <strong>{symbol}</strong>. This
            may be because the stock is not listed on NSE/BSE or Yahoo Finance
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-muted-foreground">
            Source: Yahoo Finance ({data.symbol})
          </span>
          <Button
            variant="outline"
            size="sm"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card>
              ...
              <CardContent>
                ...
                <Row
                  ...
                  value={fmt(summary.priceToSalesTrailing12Months)}
                />
                <Row
                  label="Enterprise Value"
                  value={crore(stats.enterpriseValue)}
                />
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <Row
                  label="Profit Margin"
                  value={pct(fin.profitMargins)}
                  green
                />
                <Row
                  label="Operating Margin"
                  value={pct(fin.operatingMargins)}
                  green
                />
            ...
            <Card>
              ...
              <CardContent>
                <Row
                  ...
                  value={pct(fin.revenueGrowth)}
                  green
                />
                <Row
                  label="Earnings Growth (YoY)"
                  value={pct(fin.earningsGrowth)}
                  green
                />
                <Row
                  label="Earnings Growth (Q)"
                  value={pct(fin.earningsQuarterlyGrowth)}
                  green
                />
                <Row label="Total Revenue" value={crore(fin.totalRevenue)} />
                <Row
                  label="Revenue Per Share"
                  value={fmt(fin.revenuePerShare)}
                />
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <Row
                  ...
                  value={crore(fin.operatingCashflow)}
                />
                      ...
                      {incomeY.map((item: any, i: number) => (
                        ...
                        >
                          ...
                          <td className="text-right py-2">
                            {crore(item.totalRevenue)}
                          </td>
                          <td className="text-right py-2">
                            {crore(item.grossProfit)}
                          </td>
                          <td
                            className={`text-right py-2 ${item.netIncome?.raw >= 0 ? "text-green-500" : "text-red-500"}`}
                          >
                            {crore(item.netIncome)}
                          </td>
                          <td className="text-right py-2">
                            {crore(item.operatingIncome)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                    ...
                    <tbody>
                      {balance.map((item: any, i: number) => (
                        ...
                        >
                          ...
                          <td className="text-right py-2">
                            {crore(item.totalAssets)}
                          </td>
                          <td className="text-right py-2">
                            {crore(item.totalDebt || item.longTermDebt)}
                          </td>
                          <td className="text-right py-2">
                            {crore(item.cash)}
                          </td>
                          <td className="text-right py-2">
                            {crore(item.totalStockholderEquity)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                    ...
                    <tbody>
                      {cashflow.map((item: any, i: number) => (
                        ...
                        >
                          ...
                          <td
                            className={`text-right py-2 ${item.totalCashFromOperatingActivities?.raw >= 0 ? "text-green-500" : "text-red-500"}`}
                          >
                            {crore(item.totalCashFromOperatingActivities)}
                          </td>
                          <td
                            className={`text-right py-2 ${item.totalCashflowsFromInvestingActivities?.raw >= 0 ? "text-green-500" : "text-red-500"}`}
                          >
                            {crore(item.totalCashflowsFromInvestingActivities)}
                          </td>
                          <td
                            className={`text-right py-2 ${item.totalCashFromFinancingActivities?.raw >= 0 ? "text-green-500" : "text-red-500"}`}
                          >
                            {crore(item.totalCashFromFinancingActivities)}
                          </td>
                          <td className="text-right py-2">
                            {crore(item.capitalExpenditures)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
